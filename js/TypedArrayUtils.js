 ...
 */
 ...
 THREE.TypedArrayUtils.Kdtree = function (points, metric, eleSize, presorted) {
    ...
    function buildTree(points, depth, parent, pos) {
      var dim = depth % eleSize, //maybe change to 3
        median,
        node,
      ...
      if (presorted !== true) THREE.TypedArrayUtils.quicksortIP(points, eleSize, dim);
	...
	_multiplePositionsList = [];
	
	this.root = buildTree(points, 0, null, 0);
	
	delete _multiplePositionsList;
    ...
    this.nearest = function (point, maxNodes , maxDistance, conditionFunction ) {
      var i,
        result,
        bestNodes;
		
	  if (!conditionFunction) conditionFunction = function() { return true; };
      ...
      function nearestSearch(node) {
        ...
        if (node.right === null && node.left === null) {
          if ((bestNodes.size() < maxNodes || ownDistance < bestNodes.peek()[1]) && conditionFunction(node)) {
 ...
 THREE.TypedArrayUtils.Kdtree.prototype.Node = function(obj, depth, parent, pos) {
    ...
    this.depth = depth;
	this.pos = pos;
	//if the position already exists set multiPosIndex
	if (_multiplePositionsList[pos[0]] && _multiplePositionsList[pos[0]][pos[1]] && _multiplePositionsList[pos[0]][pos[1]][pos[2]]) {
		_multiplePositionsList[pos[0]][pos[1]][pos[2]]++;
		multiPosIndex = _multiplePositionsList[pos[0]][pos[1]][pos[2]];
	} else {
		if (!_multiplePositionsList[pos[0]]) {
			_multiplePositionsList[pos[0]] = [];
		}
		if (!_multiplePositionsList[pos[0]][pos[1]]) {
			_multiplePositionsList[pos[0]][pos[1]] = [];
		}
		if (!_multiplePositionsList[pos[0]][pos[1]][pos[2]]) {
			_multiplePositionsList[pos[0]][pos[1]][pos[2]] = 1;
		}
	}
 }
